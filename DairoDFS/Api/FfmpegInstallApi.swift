...
enum FfmpegInstallApi {
  ...
  static func recycle() -> ApiHttp<EmptyModel>{
    return ApiHttp<EmptyModel>(ApiConst.APP_INSTALL_FFMPEG_RECYCLE)
  }

  static func install() -> ApiHttp<EmptyModel>{
    return ApiHttp<EmptyModel>(ApiConst.APP_INSTALL_FFMPEG_INSTALL)
  }

  //当前安装进度
  static func progress() -> ApiHttp<EmptyModel>{
    return ApiHttp<EmptyModel>(ApiConst.APP_INSTALL_FFMPEG_PROGRESS)
  }
}