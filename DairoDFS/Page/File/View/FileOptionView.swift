    ...
    var body: some View {
        if !self.fileVm.isSelectMode{
            ...
            VStack(spacing: 8){
                ...
                Divider()
                HStack{
                    BottomOptionButton("全选", icon: "checklist.checked", action: self.fileVm.selectAll)
                    BottomOptionButton("复制", icon: "document.on.document", disabled: self.fileVm.selectedCount == 0, action: {
                        self.toClipboard(2)
                    })
                    BottomOptionButton("剪切", icon: "scissors", disabled: self.fileVm.selectedCount == 0, action: {
                        self.toClipboard(1)
                    })
                    BottomOptionButton("粘贴", icon: "document.on.clipboard", disabled: self.fileVm.clipboardType == 0, action: self.onClipboardClick)
                    BottomOptionButton("分享", icon: "square.and.arrow.up", disabled: self.fileVm.selectedCount == 0, action: self.onShareClick)
                }
                HStack{
                    BottomOptionButton("删除", icon: "trash", disabled: self.fileVm.selectedCount == 0, action: self.onDeleteClick)
                    BottomOptionButton("下载", icon: "square.and.arrow.down", disabled: self.fileVm.selectedCount == 0, action: self.onDownloadClick)
                    BottomOptionButton("刷新", icon: "repeat", action: self.fileVm.reload)
                    BottomOptionButton("重命名", icon: "pencil", disabled: self.fileVm.selectedCount != 1, action: self.onRenameClick)
                    BottomOptionButton("属性", icon: "info", disabled: self.fileVm.selectedCount == 0, action: self.onPropertyClick)
                }
            }
            //        this.redrawVN.value++;
        }
