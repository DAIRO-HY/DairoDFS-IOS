...
class ImageViewerDragViewModel: ObservableObject{
     ...
     * 图片宽高比
     */
    var uiImageWHRate: CGFloat = 0
    
    /**
     * 屏幕宽高比
     */
    var screenWHRate: CGFloat = 0
    ...
    //没有缩放状态下的显示宽与高
    var displayW1: CGFloat = 0
    var displayH1: CGFloat = 0
    ...
    func setUrl(_ url: String){
        ...
        if let imagePath = CacheImageHelper.getDownloadedPath(url: url, folder: CacheImage.mCacheFolder){
            self.setImage(UIImage(contentsOfFile: imagePath)!)
            return
        }
...
struct ImageViewer: View {
    ...
    init(_ vm: ImageViewerDragViewModel) {
        self.dragVM = vm
    }
